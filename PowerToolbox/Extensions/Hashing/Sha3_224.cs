...
namespace PowerToolbox.Extensions.Hashing
{
    ...
    /// SHA3 224 算法实现
    /// </summary>
    internal sealed class Sha3_224 : HashAlgorithm
    {
        ...
        protected override byte[] HashFinal()
        {
            ...
            int fullLanes = outputBytes / 8;
            for (int i = 0; i < fullLanes; i++)
            {
                WriteULongLE(state[i], hash, i * 8);
            }

            int rem = outputBytes % 8;
            if (rem is not 0)
            {
                byte[] lane = new byte[8];
                WriteULongLE(state[fullLanes], lane, 0);
                Array.Copy(lane, 0, hash, fullLanes * 8, rem);
            }
