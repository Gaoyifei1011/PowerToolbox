using System;
using System.Security.Cryptography;

namespace PowerToolbox.Extensions.Encrypt
{
    /// <summary>
    /// SM4无线局域网标准的分组数据算法。对称加密，密钥长度和分组长度均为128位。
    /// </summary>
    /// <remarks>
    /// 我国国家密码管理局在2012年公布了无线局域网产品使用的SM4密码算法——商用密码算法。
    /// 它是分组算法当中的一种，算法特点是设计简沽，结构有特点，安全高效。
    /// 数据分组长度为128比特，密钥长度为128 比特。加密算法与密钥扩展算法都采用32轮迭代结构。
    /// SM4密码算法以字节(8位)和字(32位)作为单位进行数据处理。
    /// SM4密码算法是对合运算，因此解密算法与加密算法的结构相同，只是轮密钥的使用顺序相反，解密轮密钥是加密轮密钥的逆序。
    /// </remarks>
    public class SM4Transform : ICryptoTransform
    {
        #region 常量

        private const int BLOCK_SIZE = 16;

        private static readonly byte[] Sbox =
        [
            
        ];

        private static readonly uint[] CK =
        [
            0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
            0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
            0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
            0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
            0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
            0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
            0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
            0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
        ];

        private static readonly uint[] FK =
        [
            0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
        ];

        #endregion 常量

        #region 算法

        /// <summary>
        /// roundKeys
        /// </summary>
        private readonly uint[] rk = new uint[32];

        /// <summary>
        /// 非线性替换
        /// </summary>
        private static uint Tau(uint A)
        {
            uint b0 = Sbox[A >> 24];
            uint b1 = Sbox[(A >> 16) & 0xFF];
            uint b2 = Sbox[(A >> 8) & 0xFF];
            uint b3 = Sbox[A & 0xFF];
            return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
        }

        private static uint L_ap(uint B)
        {
            return B ^ RotateLeft(B, 13) ^ RotateLeft(B, 23);
        }

        private static uint RotateLeft(uint i, int distance)
        {
            return (i << distance) | (i >> -distance);
        }

        private uint T_ap(uint Z)
        {
            return L_ap(Tau(Z));
        }

        /// <summary>
        /// Key expansion
        /// </summary>
        private void ExpandKey(bool forEncryption, byte[] key)
        {
            uint K0 = BE_To_UInt32(key, 0) ^ FK[0];
            uint K1 = BE_To_UInt32(key, 4) ^ FK[1];
            uint K2 = BE_To_UInt32(key, 8) ^ FK[2];
            uint K3 = BE_To_UInt32(key, 12) ^ FK[3];

            if (forEncryption)
            {
                rk[0] = K0 ^ T_ap(K1 ^ K2 ^ K3 ^ CK[0]);
                rk[1] = K1 ^ T_ap(K2 ^ K3 ^ rk[0] ^ CK[1]);
                rk[2] = K2 ^ T_ap(K3 ^ rk[0] ^ rk[1] ^ CK[2]);
                rk[3] = K3 ^ T_ap(rk[0] ^ rk[1] ^ rk[2] ^ CK[3]);
                for (int i = 4; i < 32; ++i)
                {
                    rk[i] = rk[i - 4] ^ T_ap(rk[i - 3] ^ rk[i - 2] ^ rk[i - 1] ^ CK[i]);
                }
            }
            else
            {
                rk[31] = K0 ^ T_ap(K1 ^ K2 ^ K3 ^ CK[0]);
                rk[30] = K1 ^ T_ap(K2 ^ K3 ^ rk[31] ^ CK[1]);
                rk[29] = K2 ^ T_ap(K3 ^ rk[31] ^ rk[30] ^ CK[2]);
                rk[28] = K3 ^ T_ap(rk[31] ^ rk[30] ^ rk[29] ^ CK[3]);
                for (int i = 27; i >= 0; --i)
                {
                    rk[i] = rk[i + 4] ^ T_ap(rk[i + 3] ^ rk[i + 2] ^ rk[i + 1] ^ CK[31 - i]);
                }
            }
        }

        internal static uint BE_To_UInt32(byte[] bs, int off)
        {
            return ((uint)bs[off] << 24) | ((uint)bs[off + 1] << 16) | ((uint)bs[off + 2] << 8) | bs[off + 3];
        }

        internal static void UInt_To_BE(uint n, byte[] bs, int off)
        {
            bs[off] = (byte)(n >> 24);
            bs[off + 1] = (byte)(n >> 16);
            bs[off + 2] = (byte)(n >> 8);
            bs[off + 3] = (byte)n;
        }

        /// <summary>
        /// Linear substitution L
        /// </summary>
        private static uint L(uint B)
        {
            return B ^ RotateLeft(B, 2) ^ RotateLeft(B, 10) ^ RotateLeft(B, 18) ^ RotateLeft(B, 24);
        }

        /// <summary>
        /// Mixer-substitution T
        /// </summary>
        private static uint T(uint Z)
        {
            return L(Tau(Z));
        }

        #endregion 算法

        #region 属性

        /// <summary>
        /// 获取一个值，该值指示是否可重复使用当前转换。
        /// </summary>
        public bool CanReuseTransform => false;

        /// <summary>
        /// 获取一个值，该值指示是否可以转换多个块。
        /// </summary>
        public bool CanTransformMultipleBlocks => true;

        /// <summary>
        /// 获取输入块大小。
        /// </summary>
        public int InputBlockSize => BLOCK_SIZE;

        /// <summary>
        /// 获取输出块大小。
        /// </summary>
        public int OutputBlockSize => BLOCK_SIZE;

        private readonly byte[] _buffer = new byte[BLOCK_SIZE];

        #endregion 属性

        #region 构造

        /// <summary>实例化转换器</summary>
        public SM4Transform(byte[] key, byte[] iv, bool encryptMode)
        {
            if (key is null || key.Length is not 16)
            {
                throw new ArgumentException("Key must be a 16-byte array.", nameof(key));
            }

            if (iv is null && iv.Length is not 16)
            {
                throw new ArgumentException("IV must be a 16-byte array.", nameof(key));
            }

            ExpandKey(encryptMode, key);

            if (iv is not null)
            {
                Array.Copy(iv, _buffer, BLOCK_SIZE);
            }
        }

        /// <summary>销毁</summary>
        public void Dispose()
        {
        }

        #endregion 构造

        /// <summary>
        /// 块加密数据，传入缓冲区必须是整块数据
        /// </summary>
        public int EncryptData(byte[] inputBuffer, int inputOffset, byte[] outputBuffer, int outputOffset)
        {
            uint X0 = BE_To_UInt32(inputBuffer, inputOffset);
            uint X1 = BE_To_UInt32(inputBuffer, inputOffset + 4);
            uint X2 = BE_To_UInt32(inputBuffer, inputOffset + 8);
            uint X3 = BE_To_UInt32(inputBuffer, inputOffset + 12);

            for (int i = 0; i < 32; i += 4)
            {
                X0 ^= T(X1 ^ X2 ^ X3 ^ rk[i]);  // F0
                X1 ^= T(X2 ^ X3 ^ X0 ^ rk[i + 1]);  // F1
                X2 ^= T(X3 ^ X0 ^ X1 ^ rk[i + 2]);  // F2
                X3 ^= T(X0 ^ X1 ^ X2 ^ rk[i + 3]);  // F3
            }

            UInt_To_BE(X3, outputBuffer, outputOffset);
            UInt_To_BE(X2, outputBuffer, outputOffset + 4);
            UInt_To_BE(X1, outputBuffer, outputOffset + 8);
            UInt_To_BE(X0, outputBuffer, outputOffset + 12);

            return BLOCK_SIZE;
        }

        /// <summary>
        /// 转换输入字节数组的指定区域，并将所得到的转换复制到输出字节数组的指定区域
        /// </summary>
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            if (inputCount % BLOCK_SIZE is not 0)
            {
                throw new ArgumentException("Input count must be equal to block size.", nameof(inputCount));
            }

            int blocks = inputCount / InputBlockSize;
            while (blocks > 0)
            {
                EncryptData(inputBuffer, inputOffset, outputBuffer, outputOffset);
                blocks--;
                inputOffset += InputBlockSize;
                outputOffset += OutputBlockSize;
            }

            return inputCount / InputBlockSize * OutputBlockSize;
        }

        /// <summary>
        /// 转换指定字节数组的指定区域
        /// </summary>
        public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            if (inputCount is 0)
            {
                return [];
            }

            int blocks = inputCount / InputBlockSize;
            byte[] output = new byte[blocks * OutputBlockSize];
            TransformBlock(inputBuffer, inputOffset, inputCount, output, 0);
            return output;
        }
    }
}
