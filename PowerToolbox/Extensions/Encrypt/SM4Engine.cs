using System;

namespace PowerToolbox.Extensions.Encrypt
{
    /// <summary>
    /// SM4分组密码核心实现
    /// 提供密钥扩展和单块加密/解密
    /// </summary>
    internal static class SM4Engine
    {
        // System parameter FK
        private static readonly uint[] FK =
        [
            0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
        ];

        // fixed parameter CK
        private static readonly uint[] CK =
        [
            0x00070e15,0x1c232a31,0x383f464d,0x545b6269,
            0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,
            0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,
            0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,
            0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,
            0x30373e45,0x4c535a61,0x686f767d,0x848b9299,
            0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,
            0x10171e25,0x2c333a41,0x484f565d,0x646b7279
        ];

        // S-box
        private static readonly byte[] Sbox =
        [
            
        ];

        /// <summary>
        /// Key expansion: input 16 bytes key -> 32 round keys (uint)
        /// </summary>
        public static uint[] KeyExpansion(byte[] key)
        {
            if (key is null || key.Length is not 16)
            {
                throw new ArgumentException("SM4 key must be 16 bytes.");
            }

            uint[] MK = new uint[4];
            for (int i = 0; i < 4; i++)
            {
                MK[i] = BytesToUInt(key, i * 4);
            }

            uint[] K = new uint[36];
            for (int i = 0; i < 4; i++)
            {
                K[i] = MK[i] ^ FK[i];
            }

            for (int i = 0; i < 32; i++)
            {
                uint tmp = K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i];
                tmp = KeyTFunction(tmp);
                K[i + 4] = K[i] ^ tmp;
            }

            uint[] rk = new uint[32];
            for (int i = 0; i < 32; i++)
            {
                rk[i] = K[i + 4];
            }

            return rk;
        }

        /// <summary>
        /// Encrypt one 16-byte block
        /// </summary>
        public static void EncryptBlock(byte[] input, int inOff, uint[] rk, byte[] output, int outOff)
        {
            uint[] X = new uint[36];
            for (int i = 0; i < 4; i++)
            {
                X[i] = BytesToUInt(input, inOff + i * 4);
            }

            for (int i = 0; i < 32; i++)
            {
                uint tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i];
                tmp = TFunction(tmp);
                X[i + 4] = X[i] ^ tmp;
            }

            // reverse order
            UIntToBytes(X[35], output, outOff);
            UIntToBytes(X[34], output, outOff + 4);
            UIntToBytes(X[33], output, outOff + 8);
            UIntToBytes(X[32], output, outOff + 12);
        }

        /// <summary>
        /// Decrypt one 16-byte block (round keys in reverse)
        /// </summary>
        public static void DecryptBlock(byte[] input, int inOff, uint[] rk, byte[] output, int outOff)
        {
            uint[] X = new uint[36];
            for (int i = 0; i < 4; i++)
            {
                X[i] = BytesToUInt(input, inOff + i * 4);
            }

            for (int i = 0; i < 32; i++)
            {
                uint tmp = X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[31 - i];
                tmp = TFunction(tmp);
                X[i + 4] = X[i] ^ tmp;
            }

            // reverse order
            UIntToBytes(X[35], output, outOff);
            UIntToBytes(X[34], output, outOff + 4);
            UIntToBytes(X[33], output, outOff + 8);
            UIntToBytes(X[32], output, outOff + 12);
        }

        /// <summary>
        /// T function for encryption/decryption (tau then L)
        /// </summary>
        private static uint TFunction(uint x)
        {
            uint b = Tau(x);
            return b ^ Rotl(b, 2) ^ Rotl(b, 10) ^ Rotl(b, 18) ^ Rotl(b, 24);
        }

        /// <summary>
        /// Key T' with different linear transform (rot 13 and 23)
        /// </summary>
        private static uint KeyTFunction(uint x)
        {
            uint b = Tau(x);
            return b ^ Rotl(b, 13) ^ Rotl(b, 23);
        }

        /// <summary>
        /// Tau: S-box substitution on 32-bit word
        /// </summary>
        private static uint Tau(uint a)
        {
            byte[] b = [Sbox[(a >> 24) & 0xFF], Sbox[(a >> 16) & 0xFF], Sbox[(a >> 8) & 0xFF], Sbox[a & 0xFF]];
            return ((uint)b[0] << 24) | ((uint)b[1] << 16) | ((uint)b[2] << 8) | b[3];
        }

        /// <summary>
        /// util: bytes<->uint big-endian
        /// </summary>
        private static uint BytesToUInt(byte[] b, int off)
        {
            return ((uint)b[off] << 24) | ((uint)b[off + 1] << 16) | ((uint)b[off + 2] << 8) | b[off + 3];
        }

        private static void UIntToBytes(uint n, byte[] b, int off)
        {
            b[off] = (byte)(n >> 24);
            b[off + 1] = (byte)(n >> 16);
            b[off + 2] = (byte)(n >> 8);
            b[off + 3] = (byte)(n);
        }

        private static uint Rotl(uint x, int r)
        {
            return (x << r) | (x >> (32 - r));
        }
    }
}
