...
namespace PowerToolbox.Extensions.Encrypt
{
    ...
    public sealed class CbcTransform : ICryptoTransform
    {
        ...
        public CbcTransform(ICryptoTransform transform, byte[] iv, Boolean encryptMode)
        {
            ...
            if (iv is null || iv.Length != transform.InputBlockSize)
            {
        ...
        {
            if (inputCount % InputBlockSize is not 0)
            {
        ...
        {
            byte[] imm = new byte[InputBlockSize];
            ...
            if (signalFinalBlock)
            {
                byte[] lastBlock = _transform.TransformFinalBlock(imm, 0, InputBlockSize);
