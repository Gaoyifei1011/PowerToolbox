...
namespace PowerToolbox.Views.Pages
{
    ...
    internal sealed partial class FileNamePage : Page, INotifyPropertyChanged
    {
        ...
        protected override async void OnDrop(Microsoft.UI.Xaml.DragEventArgs args)
        {
            ...
            try
            {
                if (await GetDragDropSelectedFilesAsync(args.DataView) is List<string> fileList && fileList.Count > 0 && await GetNeedConvertFileListAsync(fileList) is List<OldAndNewNameModel> fileNameList && fileNameList.Count > 0)
                {
        ...
        private async void OnSelectFileClicked(object sender, RoutedEventArgs args)
        {
            ...
            if (openFileDialog.ShowDialog() is DialogResult.OK)
            {
                IsOperationFailed = false;
                OperationFailedList.Clear();
                if (await GetNeedConvertFileListAsync([.. openFileDialog.FileNames]) is List<OldAndNewNameModel> fileNameList && fileNameList.Count > 0)
                {
                    AddToFileNamePage(fileNameList);
                }
            }
