...
namespace PowerToolbox.Views.Pages
{
    ...
    internal sealed partial class AdvancedSystemOptionsPersonalizationPage : Page, INotifyPropertyChanged
    {
        ...
        private bool IsRebuildingIconCache
        {
        ...
        private DesktopIconSettingsModel SelectedIconItem
        {
        ...
        private ComboBoxItemModel RightClickMenuStyle
        {
        ...
        private ComboBoxItemModel FileExplorerStyle
        {
        ...
        private bool IsSyncProviderNotificationsEnabled
        {
        ...
        private ComboBoxItemModel FileExplorerHomePosition
        {
        ...
        private bool IsShortcutWithoutShortcutTextEnabled
        {
        ...
        private ComboBoxItemModel VisualEffectsPlan
        {
        ...
        private bool IsUpdatingVisualEffects
        {
        ...
        private async void OnDesktopIconDisplayExecuteRequested(object sender, ExecuteRequestedEventArgs args)
        {
            if (args.Parameter is DesktopIconDisplayModel desktopIconDisplay)
            {
                ...
                await SetDesktopIconVisibilityAsync(desktopIconDisplay.IconTag, desktopIconDisplay.IsIconVisible);
                desktopIconDisplay.IsIconVisible = await GetDesktopIconVisibilityAsync(desktopIconDisplay.IconTag);
            }
        }
        ...
        private async void OnChangeDesktopIconClicked(object sender, RoutedEventArgs args)
        {
            ...
            {
                string iconRegistryValuePath = await ChangeDesktopIconAsync(desktopIconSettings.IconTag, desktopIconSettings.IconLocationPath, desktopIconSettings.IconRegistryKeyPath, desktopIconSettings.IconIndex);
                if (!string.IsNullOrEmpty(iconRegistryValuePath))
                {
                    desktopIconSettings.IconImage = await UpdateIconImageAsync(iconRegistryValuePath);
                }
            }
        }
        ...
        private async void OnRestoreDefualtIconClicked(object sender, RoutedEventArgs args)
        {
            if (DesktopIconSettingsGridView.SelectedItem is DesktopIconSettingsModel desktopIconSettings)
            {
                string iconRegistryValuePath = await RestoreDefualtIconAsync(desktopIconSettings.IconTag, desktopIconSettings.IconRegistryKeyPath, desktopIconSettings.IconIndex);
                if (!string.IsNullOrEmpty(iconRegistryValuePath))
                {
                    desktopIconSettings.IconImage = await UpdateIconImageAsync(iconRegistryValuePath);
                }
        ...
        private async void OnRightClickMenuStyleSelectionChanged(object sender, SelectionChangedEventArgs args)
        {
            if (sender is ComboBox comboBox && !Equals(RightClickMenuStyle, comboBox.SelectedItem))
            {
                ...
                {
                    await SetRightClickMenuStyleAsync(RightClickMenuStyle.SelectedValue);
                }
        ...
        private async void OnFileExplorerStyleSelectionChanged(object sender, SelectionChangedEventArgs args)
        {
            if (sender is ComboBox comboBox && !Equals(FileExplorerStyle, comboBox.SelectedItem))
            {
                ...
                {
                    await SetFileExplorerStyleAsync(FileExplorerStyle.SelectedValue);
                }
        ...
        private async void OnSyncProviderNotificationsToggled(object sender, RoutedEventArgs args)
        {
            if (sender is ToggleSwitch toggleSwitch)
            {
                ...
                IsSyncProviderNotificationsEnabled = await GetIsSyncProviderNotificationsEnabledAsync();
            }
        }
        ...
        private async void OnFileExplorerHomePositionSelectionChanged(object sender, SelectionChangedEventArgs args)
        {
            if (sender is ComboBox comboBox && !Equals(FileExplorerHomePosition, comboBox.SelectedItem))
            {
                ...
                {
                    await SetFileExplorerHomePositionAsync(FileExplorerHomePosition.SelectedValue);
                }

                FileExplorerHomePosition = await GetFileExplorerHomePositionAsync();
            }
        }
        ...
        private async Task InitializeSettingsOptionsAsync()
        {
            if (RuntimeHelper.IsElevated)
            {
                ...
                bool controlPanelDesktopIconVisible = await GetDesktopIconVisibilityAsync("ControlPanel");
                bool libraryDesktopIconVisible = await GetDesktopIconVisibilityAsync("Library");
                bool networkDesktopIconVisible = await GetDesktopIconVisibilityAsync("Network");
                bool recycleBinDesktopIconVisible = await GetDesktopIconVisibilityAsync("RecycleBin");
                bool thisPCDesktopIconVisible = await GetDesktopIconVisibilityAsync("ThisPC");
                bool userFolderDesktopIconVisible = await GetDesktopIconVisibilityAsync("UserFolder");
                ...
                if (RuntimeHelper.IsWindows11)
                {
                    ...
                    bool homeNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("Home");
                    bool libraryNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("Library");
                    bool linuxNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("Linux");
                    bool photoGalleryNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("PhotoGallery");
                    bool recycleBinNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("RecycleBin");
                    bool removableDeviceNavigationPaneIconVisible = await GetNavigationPaneIconVisibilityAsync("RemovableDevice");
        ...
        /// 获取桌面图标显示状态
        /// </summary>
        private async Task<bool> GetDesktopIconVisibilityAsync(string iconTag)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                switch (iconTag)
                {
                    case "ControlPanel":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", controlPanelPath);
                            isIconVisible = iconValue.HasValue && iconValue.Value is 0;
                            break;
                        }
                    case "Library":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", libraryPath);
                            isIconVisible = iconValue.HasValue && iconValue.Value is 0;
                            break;
                        }
                    case "Network":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", networkPath);
                            isIconVisible = iconValue.HasValue && iconValue.Value is 0;
                            break;
                        }
                    case "RecycleBin":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", recycleBinPath);
                            isIconVisible = !iconValue.HasValue || iconValue.Value is 0;
                            break;
                        }
                    case "ThisPC":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", thisPCPath);
                            isIconVisible = iconValue.HasValue && iconValue.Value is 0;
                            break;
                        }
                    case "UserFolder":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", userFolderPath);
                            isIconVisible = iconValue.HasValue && iconValue.Value is 0;
                            break;
                        }
                }
        ...
        /// 设置桌面图标显示状态
        /// </summary>
        private async Task SetDesktopIconVisibilityAsync(string iconTag, bool isIconVisible)
        {
            if (string.IsNullOrEmpty(iconTag))
            {
                return;
            ...
            await Task.Run(() =>
            {
                switch (iconTag)
                {
                    case "ControlPanel":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", controlPanelPath, isIconVisible ? 0 : 1);
                            break;
                        }
                    case "Library":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", libraryPath, isIconVisible ? 0 : 1);
                            break;
                        }
                    case "Network":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", networkPath, isIconVisible ? 0 : 1);
                            break;
                        }
                    case "RecycleBin":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", recycleBinPath, isIconVisible ? 0 : 1);
                            break;
                        }
                    case "ThisPC":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", thisPCPath, isIconVisible ? 0 : 1);
                            break;
                        }
                    case "UserFolder":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel", userFolderPath, isIconVisible ? 0 : 1);
                            break;
                        }
                }
        ...
        /// 获取导航窗格图标显示状态
        /// </summary>
        private async Task<bool> GetNavigationPaneIconVisibilityAsync(string iconTag)
        {
            if (string.IsNullOrEmpty(iconTag))
            {
                return default;
            ...
            return await Task.Run(() =>
            {
                ...
                switch (iconTag)
                {
                    case "Home":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", homePath), "System.IsPinnedToNameSpaceTree");
                            isIconVisible = !iconValue.HasValue || iconValue.Value is not 0;
                            break;
                        }
                    case "Library":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.ClassesRoot, string.Format(@"CLSID\{0}", libraryPath), "System.IsPinnedToNameSpaceTree");
                            isIconVisible = !iconValue.HasValue || iconValue.Value is not 0;
                            break;
                        }
                    case "Linux":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", linuxPath), "System.IsPinnedToNameSpaceTree");
                            isIconVisible = !iconValue.HasValue || iconValue.Value is not 0;
                            break;
                        }
                    case "PhotoGallery":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", photoGalleryPath), "System.IsPinnedToNameSpaceTree");
                            isIconVisible = !iconValue.HasValue || iconValue.Value is not 0;
                            break;
                        }
                    case "RecycleBin":
                        {
                            int? iconValue = RegistryHelper.ReadRegistryKey<int?>(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", recycleBinPath), "System.IsPinnedToNameSpaceTree");
                            isIconVisible = !iconValue.HasValue || iconValue.Value is not 0;
                            break;
                        }
                    case "RemovableDevice":
                        {
                            isIconVisible = RegistryHelper.IsRegistryKeyExisted(Registry.LocalMachine, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\DelegateFolders\{F5FB2C77-0E2F-4A16-A381-3E560C68BC83}");
                            break;
                        }
                }
                return isIconVisible;
        ...
        /// 设置导航窗格图标显示状态
        /// </summary>
        private async Task SetNavigationPaneIconVisibilityAsync(string iconTag, bool isIconVisible)
        {
            if (string.IsNullOrEmpty(iconTag))
            {
                return;
            ...
            await Task.Run(() =>
            {
                switch (iconTag)
                {
                    case "Home":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", homePath), "System.IsPinnedToNameSpaceTree", isIconVisible);
                            break;
                        }
                    case "Library":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.ClassesRoot, string.Format(@"CLSID\{0}", libraryPath), "System.IsPinnedToNameSpaceTree", isIconVisible);
                            break;
                        }
                    case "Linux":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", linuxPath), string.Empty, "Linux");
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", linuxPath), "System.IsPinnedToNameSpaceTree", isIconVisible);
                            break;
                        }
                    case "PhotoGallery":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", photoGalleryPath), "System.IsPinnedToNameSpaceTree", isIconVisible);
                            break;
                        }
                    case "RecycleBin":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, string.Format(@"Software\Classes\CLSID\{0}", recycleBinPath), "System.IsPinnedToNameSpaceTree", isIconVisible);
                            break;
                        }
                    case "RemovableDevice":
                        {
        ...
        private async Task RebuildIconCacheAsync()
        {
            await Task.Run(async () =>
            {
                ...
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(PowerToolbox), nameof(AdvancedSystemOptionsPersonalizationPage), nameof(RebuildIconCacheAsync), 1, e);
                }
                finally
                {
                    try
                    ...
                    {
                        LogService.WriteLog(TraceEventType.Error, nameof(PowerToolbox), nameof(AdvancedSystemOptionsPersonalizationPage), nameof(RebuildIconCacheAsync), 2, e);
                    }
                }
            });
        }
        ...
        /// 修改桌面图标
        /// </summary>
        private async Task<string> ChangeDesktopIconAsync(string iconTag, string iconLocationPath, string iconRegistryKeyPath, int iconIndex)
        {
            if (string.IsNullOrEmpty(iconTag) || string.IsNullOrEmpty(iconLocationPath) || string.IsNullOrEmpty(iconRegistryKeyPath))
            {
                return default;
            }

            StringBuilder desktopIconStringBuilder = new(iconLocationPath, 260);
            int desktopIconIndex = iconIndex;
            ...
            if (Shell32Library.PickIconDlg((nint)MainWindow.Current.AppWindow.Id.Value, desktopIconStringBuilder, desktopIconStringBuilder.Capacity, ref desktopIconIndex))
            {
                await Task.Run(() =>
                {
                    switch (iconTag)
                    {
                        case "ThisPC":
                            {
                                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, string.Empty, string.Format("{0},{1}", desktopIconStringBuilder.ToString(), desktopIconIndex));
                                iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, string.Empty);
                                break;
                            }
                        case "UserFolder":
                            {
                        ...
                        case "RecycleBinFull":
                            {
                                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, "full", string.Format("{0},{1}", desktopIconStringBuilder.ToString(), desktopIconIndex));
                                iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, "full");
                                break;
                            }
                        case "RecycleBinEmpty":
                            {
                                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, "empty", string.Format("{0},{1}", desktopIconStringBuilder.ToString(), desktopIconIndex));
                                iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, "empty");
                                break;
                            }
                    }
                    Shell32Library.SHChangeNotify(SHCNE.SHCNE_ASSOCCHANGED, SHCNF.SHCNF_IDLIST | SHCNF.SHCNF_FLUSH, 0, 0);
            ...
            return iconRegistryValuePath;
        }

        /// <summary>
        /// 还原默认图标
        /// </summary>
        private async Task<string> RestoreDefualtIconAsync(string iconTag, string iconRegistryKeyPath, int iconIndex)
        {
            if (string.IsNullOrEmpty(iconTag) || string.IsNullOrEmpty(iconRegistryKeyPath))
            {
                return default;
            }
            ...
            return await Task.Run(() =>
            {
                ...
                switch (iconTag)
                {
                    case "ThisPC":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, string.Empty, @"%SystemRoot%\System32\imageres.dll,-109");
                            iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, string.Empty);
                            break;
                        }
                    case "UserFolder":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, string.Empty, @"%SystemRoot%\System32\imageres.dll,-123");
                            iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, string.Empty);
                            break;
                        }
                    case "Network":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, string.Empty, @"%SystemRoot%\System32\imageres.dll,-25");
                            iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, string.Empty);
                            break;
                        }
                    case "RecycleBinFull":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, "full", @"%SystemRoot%\System32\imageres.dll,-54");
                            iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, "full");
                            break;
                        }
                    case "RecycleBinEmpty":
                        {
                            RegistryHelper.SaveRegistryKey(Registry.CurrentUser, iconRegistryKeyPath, "empty", @"%SystemRoot%\System32\imageres.dll,-55");
                            iconRegistryValuePath = RegistryHelper.ReadRegistryKey<string>(Registry.CurrentUser, iconRegistryKeyPath, "empty");
                            break;
                        }
                }
        ...
        /// 更新应用图标
        /// </summary>
        private async Task<BitmapImage> UpdateIconImageAsync(string iconRegistryValuePath)
        {
            if (string.IsNullOrEmpty(iconRegistryValuePath))
            {
                return default;
            ...
            if (await GetShellIconAsync(iconLocationPath, iconIndex) is MemoryStream iconMemoryStream)
            {
                try
                {
                    BitmapImage bitmapImage = new();
                ...
                {
                    LogService.WriteLog(TraceEventType.Error, nameof(PowerToolbox), nameof(AdvancedSystemOptionsPersonalizationPage), nameof(UpdateIconImageAsync), 1, e);
                    return null;
                }
            }
            else
        ...
        /// 显示 / 隐藏桌面快捷键头
        /// </summary>
        private async Task ShowOrHideShowDesktopShortcutArrowAsync(bool isShow)
        {
            await Task.Run(async () =>
            {
                if (RuntimeHelper.IsElevated)
        ...
        /// 设置右键菜单样式
        /// </summary>
        private async Task SetRightClickMenuStyleAsync(object rightClickMenuStyle)
        {
            if (rightClickMenuStyle is null)
            {
                return;
            }
        ...
        /// 获取经典右键菜单状态
        /// </summary>
        private async Task<bool> GetIsClassicRightClickMenuExistedAsync()
        {
            return await Task.Run(() =>
            {
        ...
        /// 设置资源管理器样式
        /// </summary>
        private async Task SetFileExplorerStyleAsync(object fileExplorerStyle)
        {
            if (fileExplorerStyle is null)
            {
                return;
            }
            ...
            await Task.Run(() =>
            {
                if (fileExplorerStyle is "Windows11ModernFileExplorer")
                {
                ...
                else if (fileExplorerStyle is "Windows10ClassicFileExplorer")
                {
        ...
        /// 获取经典资源管理器状态
        /// </summary>
        private async Task<bool> GetIsClassicFileExplorerExistedAsync()
        {
            return await Task.Run(() =>
            {
        ...
        /// 获取供应商同步通知状态
        /// </summary>
        private async Task<bool> GetIsSyncProviderNotificationsEnabledAsync()
        {
            return await Task.Run(() =>
            {
                if (RuntimeHelper.IsElevated)
        ...
        /// 获取资源管理器首页位置
        /// </summary>
        private async Task<ComboBoxItemModel> GetFileExplorerHomePositionAsync()
        {
            return await Task.Run(() =>
            {
        ...
        /// 设置资源管理器首页位置
        /// </summary>
        private async Task SetFileExplorerHomePositionAsync(object fileExplorerHomePosition)
        {
            if (fileExplorerHomePosition is null)
            {
                return;
            }
            ...
            await Task.Run(() =>
            {
                if (fileExplorerHomePosition is "ThisPC")
                {
                ...
                else if (fileExplorerHomePosition is "Home")
                {
                ...
                else if (fileExplorerHomePosition is "Downloads")
                {
        ...
        /// <summary>
        /// 获取创建快捷方式不显示快捷方式文字选项设置
        /// </summary>
        private async Task<bool> GetIsShortcutWithoutShortcutTextEnabledAsync()
        {
            return await Task.Run(() =>
            {
        ...
        /// <summary>
        /// 设置创建快捷方式不显示快捷方式文字选项设置
        /// </summary>
        private async Task SetIsShortcutWithoutShortcutTextEnabledAsync(bool isShortcutWithoutShortcutTextEnabled)
        {
            await Task.Run(() =>
            {
        ...
        /// 获取视觉效果设置
        /// </summary>
        private async Task<VisualEffects> GetVisualEffectsAsync()
        {
            return await Task.Run(() =>
            {
        ...
        /// 保存视觉效果设置
        /// </summary>
        private async Task SetVisualEffectsAsync(VisualEffects visualEffects)
        {
            ...
            await Task.Run(() =>
            {
                if (visualEffects.VisualEffectsPlan >= 0 && visualEffects.VisualEffectsPlan <= 3)
                {
                    RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects", "VisualFXSetting", visualEffects.VisualEffectsPlan);
                }
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\DWM", "AlwaysHibernateThumbnails", Convert.ToInt32(visualEffects.SaveTaskbarThumbnailPreview));
                User32Library.SystemParametersInfo(SPI.SPI_SETCLIENTAREAANIMATION, 0, Convert.ToInt32(visualEffects.AnimationControlsAndElementsInsideWindow), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETMENUANIMATION, 0, Convert.ToInt32(visualEffects.FadeinAndOutOrSlideMenuToView), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETCOMBOBOXANIMATION, 0, Convert.ToInt32(visualEffects.SlideToOpenCombobox), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETLISTBOXSMOOTHSCROLLING, 0, Convert.ToInt32(visualEffects.SmoothScrollListbox), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETFONTSMOOTHING, Convert.ToUInt32(visualEffects.SmoothScreenFontEdges), 0, SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\DWM", "EnableAeroPeek", Convert.ToInt32(visualEffects.EnablePeek));
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "TaskbarAnimations", Convert.ToInt32(visualEffects.TaskbarAnimations));
                User32Library.SystemParametersInfo(SPI.SPI_SETDRAGFULLWINDOWS, Convert.ToUInt32(visualEffects.ShowWindowContentsWhileDragging), 0, SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "IconsOnly", visualEffects.ShowThumbnail ? 0 : 1);
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ListviewAlphaSelect", Convert.ToInt32(visualEffects.ShowSemitransparentSelectedRectangle));
                User32Library.SystemParametersInfo(SPI.SPI_SETDROPSHADOW, 0, Convert.ToInt32(visualEffects.ShowShadowUnderWindow), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETSELECTIONFADE, 0, Convert.ToInt32(visualEffects.FadeoutMenuAfterClicking), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETTOOLTIPANIMATION, 0, Convert.ToInt32(visualEffects.FadeinFadeoutOrSlideToolTipInView), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                User32Library.SystemParametersInfo(SPI.SPI_SETCURSORSHADOW, 0, Convert.ToInt32(visualEffects.ShowShadowUnderMousePointer), SPIF.SPIF_UPDATEINIFILE | SPIF.SPIF_SENDCHANGE);
                RegistryHelper.SaveRegistryKey(Registry.CurrentUser, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ListviewShadow", Convert.ToInt32(visualEffects.UseShadowForIconLabelsOnDesktop));
                SetSystemParametersInfoAnimationInfoValue(SPI.SPI_SETANIMATION, new()
                {
                    cbSize = (uint)Marshal.SizeOf<ANIMATIONINFO>(),
                    iMinAnimate = Convert.ToInt32(visualEffects.ShowAnimationWhenMaximizingOrMinimizing)
                });
                Shell32Library.SHChangeNotify(SHCNE.SHCNE_ASSOCCHANGED, SHCNF.SHCNF_IDLIST | SHCNF.SHCNF_FLUSH, 0, 0);
            });
        }
        ...
        /// 更新视觉效果设置
        /// </summary>
        private void UpdateVisualEffects(VisualEffects visualEffects)
        {
            if (visualEffects is null)
            {
                return;
        ...
        {
            if (string.IsNullOrEmpty(iconStoragePath))
            {
                return default;
            }
        ...
        {
            if (string.IsNullOrEmpty(iconPathName))
            {
                return default;
            }
